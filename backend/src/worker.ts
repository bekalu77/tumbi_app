...
app.use('/api/*', async (c, next) => {
    ...
    try {
        ...
        const users = await sql`SELECT id, name, email, phone, location FROM users WHERE id = ${parseInt(decoded.id as string)}`;
...
app.post('/api/register', async (c) => {
    ...
    // Explicitly check for existing user using separate OR conditions
    // Using a safer approach for optional email
    const existing = await sql`
        SELECT id FROM users 
        WHERE phone = ANY(${variants}) 
           OR phone = ${normPhone} 
           OR (email IS NOT NULL AND LOWER(email) = LOWER(${cleanEmail || ''}))
    `;
    
    if (existing.length) return c.json({ message: 'An account with this phone or email already exists' }, 409);
    ...
    return c.json({ 
        auth: true, 
        token, 
        user: { id: String(result[0].id), name, email: cleanEmail, phone: normPhone, location } 
    });
});
...
app.post('/api/login', async (c) => {
    ...
    if (rows.length === 0) return c.json({ message: 'Invalid credentials' }, 401);
    
    const user = rows[0];
    if (!(await bcrypt.compare(password, user.password))) return c.json({ message: 'Invalid credentials' }, 401);
